...
describe('executeQuery', () => {
  ...
  it('should execute a simple query successfully', async () => {
    ...
    const response = await executeQuery(mockStorage, sparqlQuery, backendId);
  ...
  it('should throw an error if backend is not found', async () => {
    const mockStorage = createMockStorage(null); // Simulate backend not found

    await expect(executeQuery(mockStorage, sparqlQuery, 'non-existent-backend')).rejects.toThrow(
      'Backend "non-existent-backend" not found'
    );
    expect(mockStorage.getBackendById).toHaveBeenCalledWith('non-existent-backend');
  ...
  it('should include Authorization header when backend has credentials', async () => {
    ...
    await executeQuery(mockStorage, sparqlQuery, backendId);
  ...
  it('should apply variable bindings to the query', async () => {
    ...
    await executeQuery(mockStorage, queryWithVar, backendId, bindings); // Pass the correct bindings structure
  ...
  it('should re-throw errors from undici request', async () => {
    ...
    await expect(executeQuery(mockStorage, sparqlQuery, backendId)).rejects.toThrow(requestError);
    expect(mockStorage.getBackendById).toHaveBeenCalledWith(backendId);
  ...
  it('should enable timing logs when config is set', async () => {
    ...
    try {
      ...
      await executeQuery(mockStorage, sparqlQuery, backendId, { some: 'var' }); // Pass variables to trigger timing
